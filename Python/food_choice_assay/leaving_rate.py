...
from food_choice_assay.food_choice_helper import foodchoice
...
for p, prefood in enumerate(pretreatments):
    ...
    for a, assay in enumerate(assaychoices):
        for c, conc in enumerate(concentrations):
            ...
            plot_timeseries_phenix(plot_df, colour_dict, window=False, acclimtime=False,\
...
for i, maskedfilepath in enumerate(fullMetaData['filename']):
    ...
    skeletonfilepath = change_path_phenix(maskedfilepath, returnpath="skeletons")
    
    #from read_data.read import get_skeleton_data
    #skeleton_data = get_skeleton_data(skeletonfilepath)
