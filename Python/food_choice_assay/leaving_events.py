...
from visualisation.plotting_helper import hexcolours, plot_points
...
def leavingevents(df, window=50, removeNone=True, plot=True, savePath=None):
    ...
    for worm in worm_ids:
        ...
        # TODO: Do not treat foods independently! What about worms that leave to enter the other food? 
        #       Ok for leaving event selection stage, but not perfect!
        for fc, food in enumerate(foods):
            ...
            if len(leaving) > 0: 
                ...
                if len(entering) > 0:
                    # If there is an entering event
                    if len(entering) == len(leaving): 
                        # If the number of leaving & entering events are equal
                        if leaving[0] < entering[0]: 
                            # If the first event is a leaving event, do nothing
                            pass
                        elif entering[0] < leaving[0]: 
                            # If first event is an entering event, delete first entering event
                            entering = entering[1:]
                            # compare leaving duration to end of trajectory by adding end of 
                            # trajectory index as last 'entering event'
                            entering = np.insert(entering, len(entering), df_worm.index[-1])
                            
                    elif not len(entering) == len(leaving):
                        # If number of leaving & entering events are unequal
                        if leaving[0] < entering[0]: 
                            # If the first event is a leaving event, add end of trajectory index 
                            # as last 'entering event' (ie. compare to end of trajectory)
                            entering = np.insert(entering, len(entering), df_worm.index[-1])
                        elif entering[0] < leaving[0]: 
                            # If first event is an entering event, delete first entry event
                            entering = entering[1:]
                            
                else: 
                    # If one leaving but no entering events, compare to end of trajectory
                    entering = np.array([df_worm.index[-1]])
                leaving_duration = entering - leaving
                # -1 to index the frame just before the leaving event to know which food it left from
                leaving_df = df_worm.loc[leaving - 1] 
...
for i, maskedfilepath in enumerate(fullMetaData['filename']):
    ...
    # plots_out = change_path_phenix(maskedfilepath, returnpath='plots', figname='leavingTSplot.eps')
...
for i, maskedfilepath in enumerate(fullMetaData['filename']):
    ...
    coordfilepath = change_path_phenix(maskedfilepath, returnpath='coords')
    featurefilepath = change_path_phenix(maskedfilepath, returnpath='features')
    plots_out = change_path_phenix(maskedfilepath, returnpath='plots', figname='LeavingEventsOverlay.png')
    ...
    try:
        ...
        plt.close("all")
        fig, ax = plot_brightfield(maskedfilepath, frame=0, figsize=(10,10))
        ...
        fig, ax = plot_points(fig, ax, true_leaving_df[true_leaving_df.filename==maskedfilepath]['x'],\
