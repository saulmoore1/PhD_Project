...
def pair_input_files(raw_tif_dir, prob_map_dir):
    ...
    for raw_img_path in tqdm(raw_img_list):
        prob_map_name = raw_img_path.name.replace(".tif","_Object Probabilities.tif")
        prob_map_path = prob_map_dir / prob_map_name
         
        if not prob_map_path in prob_map_list:
            print("WARNING: No probability map file found for %s" % raw_img_path)
...
def overlay_probability_map_outline(raw_img_path, prob_map_path, save_dir=None, 
    ...
    prob_map_eroded = cv2.erode(prob_map, kernel=np.ones((5,5), np.uint8), iterations=1)
...
if __name__ == "__main__":
    ...
    raw_tif_dir = Path("/Users/sm5911/Documents/PanGenome/data/200827_dev_assay_optimisation_focussed/tif_images")
    prob_map_dir = Path("/Users/sm5911/Documents/PanGenome/results/200827_dev_assay_optimisation_results/object_probabilities/tif_images")
    save_dir = Path("/Users/sm5911/Documents/PanGenome/results/200827_dev_assay_optimisation_results/object_overlays")
        
    paired_files = pair_input_files(raw_tif_dir, prob_map_dir)
    
    for (raw_img_path, prob_map_path) in tqdm(paired_files):
        img = overlay_probability_map_outline(raw_img_path, 
                                              prob_map_path,
                                              save_dir)
