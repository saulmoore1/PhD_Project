...
# -*- coding: utf-8 -*-
"""
Keio Mutant Worm Screen - Response of several C. elegans neurosensory mutants to E. coli fepD
...
SAVE_DIR = "/Users/sm5911/Documents/PhD_DLBG/10_Keio_Worm_Mutants"
...
#                    bacteria_lut.values()]
#         plt.legend(markers, bacteria_lut.keys(), numpoints=1, frameon=False, loc='best', 
#                    markerscale=0.75, fontsize=8, handletextpad=0.2)
#         
...
def main():
    ...
    # subset for window 5 only - 20-30 seconds after the final BL pulse
    metadata = metadata[metadata['window']==5]
    features = features.reindex(metadata.index)
    
    strain_list = sorted(metadata['bacteria_strain'].unique())
    worm_list = sorted(metadata['worm_strain'].unique())
    ...
    for date in dates:
        ...
        sns.stripplot(x='worm_strain',
                      ...
                      hue_order=strain_list,
                      dodge=True,
                      palette=[date_lut[date]] * len(worm_list),
                      color=None,
                      marker=".",
                      edgecolor='k',
    ...
    ax.axes.set_xticklabels(worm_list, fontsize=20)
    ...
    for i, text in enumerate(ax.axes.get_xticklabels()):
        worm = text.get_text()
        if worm == 'N2':
            p = pvals.loc['speed_50th',worm+'-fepD']
    ...
    boxplot_path = plots_dir / 'speed_50th_vs_N2_on_BW.svg'
