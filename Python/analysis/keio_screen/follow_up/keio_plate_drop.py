...
SAVE_DIR = "/Users/sm5911/Documents/Keio_Plate_Drop"
...
def main():
    ...
    treatment_cols = ['plate_drop_number','bacteria_strain']
    ...
    reject, pvals_df['fepD'] = _multitest_correct(pvals_df['fepD'], multitest_method='fdr_by', fdr=0.05)
    ...
    for feature in tqdm(FEATURE_LIST):
        ...
        for i, n_drop in enumerate(drop_list):
            ...
            p = n_drop_pvals.loc[feature, 'fepD']
            text = ax.get_xticklabels()[i]
            assert text.get_text() == str(n_drop)
            p_text = sig_asterix([p])[0]
            ax.text(i+.2, 0.95, p_text, fontsize=30, ha='center', va='bottom', transform=trans)
            
        # save plot
        plt.subplots_adjust(left=0.1, right=0.95, top=0.95, bottom=0.15)
        plt.savefig(Path(SAVE_DIR) / ('plate_drop_' 
                                      + ('abs_' if ABSOLUTE_SPEED else '') 
                                      + '{}.png'.format(feature)
                                      ), dpi=DPI)
