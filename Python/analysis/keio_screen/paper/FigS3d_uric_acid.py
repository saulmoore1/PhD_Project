...
SAVE_DIR = "/Users/sm5911/Documents/PhD_DLBG/FigS3d uric acid"

uric_acid_treatment_list = ['BW',
                            'BW-NaOH',
                            'BW-uric acid-0.01',
                            'BW-uric acid-0.005',
                            'fepD',
...
def main():
    ...
    metadata = metadata[metadata['treatment'].isin(uric_acid_treatment_list)]
    assert all(t in list(metadata['treatment'].unique()) for t in uric_acid_treatment_list)
    ...
    plot_df = metadata[treatment_cols + ['treatment', 'date_yyyymmdd']
                       ].join(features[['speed_50th']]).sort_values(by='treatment', ascending=True)
    plot_df.to_csv(Path(SAVE_DIR) / 'FigS3d_data.csv')
    
    # boxplot
    lut = dict(zip(['BW','fepD'], sns.color_palette(palette='tab10', n_colors=2)))
    colour_dict = {i:(lut['fepD'] if 'fepD' in i else lut['BW']) for i in uric_acid_treatment_list}        
    ...
    sns.boxplot(x='treatment',
                y='speed_50th',
                data=plot_df, 
                order=uric_acid_treatment_list,
                palette=colour_dict,
                hue='bacteria_strain',
                showfliers=False, 
                showmeans=False,
    ...
    boxplot_path = Path(SAVE_DIR) / 'FigS3d_uric_acid.svg'
