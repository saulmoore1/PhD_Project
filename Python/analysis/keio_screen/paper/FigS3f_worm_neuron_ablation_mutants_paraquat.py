...
SAVE_DIR = '/Users/sm5911/Documents/PhD_DLBG/FigS3f worm neuron ablation mutants paraquat'
...
def main():
    ...
    metadata = metadata[metadata['worm_strain'].isin(WORM_GROUP_DICT['neuron_ablation'])]
    ...
    plot_df.to_csv(Path(SAVE_DIR) / 'FigS3f_data.csv', header=True, index=False)
    ...
    order = [w for w in WORM_GROUP_DICT['neuron_ablation'] if w in 
    ...
    plt.close('all')
    sns.set_style('ticks')
    fig = plt.figure(figsize=FIGSIZE_DICT['neuron_ablation'])
    ...
    boxplot_path = Path(SAVE_DIR) / 'FigS3f_worm_neuron_ablation_mutants_paraquat.svg'
