...
if __name__ == "__main__":
    ...
    # Path to project root containing 'AuxiliaryFiles', 'RawVideos', 'MaskedVideos' and 'Results'

    AUX_DIR = PROJECT_DIR / "AuxiliaryFiles"
    RESULTS_DIR = PROJECT_DIR / "Results"
    SAVE_DIR = Path(args.save_dir) if args.save_dir is not None else PROJECT_DIR / "Analysis"
    ...
    # args.dates # list, List of imaging dates to use for analysis. If None, all dates will be investigated
    # args.runs # list, List of imaging run numbers to use for analysis. If None, all runs will be investigated
    # args.omit_strains # List of strains in 'analyse_variables' to omit from the analysis
    # args.compile_day_summaries # bool, Compile full feature summaries from day feature summary results
    # args.impute_nans # bool, Impute NaN valus using global mean value
    # args.use_top256 # bool, Use Tierpsy Top256 features only
    # args.drop_size_features # bool, Remove size-related Tierpsy features from analysis 
    # args.align_bluelight# bool, Align bluelight conditions
    # args.add_well_annotations # bool, Add 'is_bad_well' labels from WellAnnotator GUI
    # args.max_value_cap # float, Max value limit for feature summaries
    # args.norm_features_only # bool, Use only normalised size-invariant features ('_norm') for analysis
    # args.percentile_to_use # str, Use only feature summaries for given percentile in analysis (eg. '50th')
    # args.remove_outliers # bool, Use Mahalanobis distance to exclude outliers from analysis
    # args.nan_threshold # float, Threshold proportion of NaN values to drop feature from analysis
    # args.pval_threshold # float, Threshold p-value for statistical significance
    # args.k_sig_features # int, Number of most significantly different features to plot
    # args.fdr_method # str, Benjamini-Yekutieli correction for multiple testing
    # args.lmm_random_effect # str, Random effect variable to use for linear mixed models
    # args.use_k_sig_feats_overlap # bool, Restrict significant feature set to overlap with k sig feats?
    ...
    for run in imaging_run_list:
        ...
        if not stats_table_path.exists():
