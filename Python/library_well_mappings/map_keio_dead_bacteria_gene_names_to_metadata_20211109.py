...
# -*- coding: utf-8 -*-
"""
Keio Dead (UV-killed) bacteria experiment - Map gene names for mitochondrial ETC mutants, and if 
dead or not, to metadata
...
def make_mapping_dict(path_gene_mapping=PATH_GENE_MAPPING):
    ...
    mapping_dict = {(plate, well) : (gene, dead) for plate, well, gene, dead in zip(mapping_df['source_plate'], 
...
def map_gene_name_to_metadata(metadata, mapping_dict):
    ...
    for (plate, well), (gene, dead) in mapping_dict.items():      
        ...
        metadata.loc[(metadata['source_plate_id'] == plate) & 
             (metadata['well_name'] == well), 'dead'] = dead
