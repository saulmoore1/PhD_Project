...
# -*- coding: utf-8 -*-
"""
Figure 4c - fepD mutants (round 2) - oxidative stress mutants
...
OMIT_STRAINS = ['OE acnA','OE acnB','OE fecA','OE prpB','OE_ubiC','OE_ubiF','fepD_OE_ubiC',
                'fepD_OE_ubiU','fepD_OE_ubiV','fepD_ubiC_OE_ubiF','ubiC_OE_ubiF','ubiF_OE_ubiC']

STRAIN_ORDER = ['BW','fepD','fepD; empty plasmid','fepD; OE fecA','fepD; OE acnB','fepD; OE prpB',
                'fepD; prpB; OE acnB','fepD; prpC; OE acnB']
...
def main():
    ...
    # subset for overexpression strains
    mask = np.logical_or(
        np.array(metadata['bacteria_strain'].isin(['BW','fepD','fepD; empty plasmid'])),
        np.array(pd.Series(['OE' in i for i in metadata['bacteria_strain']])))
    ...
    metadata = metadata[~metadata['bacteria_strain'].isin(OMIT_STRAINS)]

    # reindex features
    features = features.reindex(metadata.index)
        
    colours = sns.color_palette('tab10',2)
    for i in range((len(STRAIN_ORDER)-2)):
        colours.append(sns.color_palette('Greys',1)[0])
    colour_dict = dict(zip(STRAIN_ORDER, colours))
    ...
    ax.axhline(y=feat_control.median(), xmin=0, xmax=len(STRAIN_ORDER), ls='--', c='k')
